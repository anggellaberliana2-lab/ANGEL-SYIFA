...
function setupLoginForm() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(e) {
            ...
            if (email === 'baramop@gmail.com' && password === 'capybara') {
...
function setupTracking() {
    ...
    if (searchBtn) {
        searchBtn.addEventListener('click', function() {
            ...
            if (!doNumber) {
                alert('Id Kue Harus Diisi');
                return;
            }
            
            // Cari data pengiriman
            const pengiriman = dataPengiriman.find(
                item => item.idPesanan.toLowerCase() === doNumber.toLowerCase());
            ...
            if (pengiriman) {
                ...
                resultDiv.innerHTML = `
                    <div class="tracking-details">
                        ...
                        <p><strong>Id Pesanan:</strong> ${pengiriman.idPesanan}</p>
                        <p><strong>Nama Pelanggan:</strong> ${pengiriman.namaPelanggan}</p>
    ...
    if (tableBody) {
        ...
        if (addStockForm) {
            addStockForm.addEventListener('submit', function(e) {
                ...
                const idKue = document.getElementById('kodeBahan').value;
                const namaKue = document.getElementById('namaBahan').value;
                const deskripsi = parseInt(document.getElementById('stok').value);
                const status = document.getElementById('status').value;
                const stok = parseInt(document.getElementById('stok').value);
                
                // Validasi
                if (!idKue || !namaKue || !deskripsi || !status || isNaN(stok)) {
                    alert('Semua field harus diisi');
                    return;
                }
    ...
    if (tableBody) {
        ...
        dataMenu.forEach(item => {
            ...
            row.innerHTML = `
                ...
                <td>${item.stok}</td>
                <td>
                    <button class="action-btn edit-btn" data-kode="${item.idKue}">Edit</button>
                    <button class="action-btn delete-btn" data-kode="${item.idKue}">Hapus</button>
                </td>
            `;
...
function displayDummyDataInTracking() {
    ...
    if (tableBody && dataPengiriman) {
        ...
        dataPengiriman.forEach(item => {
            ...
            row.innerHTML = `
                <td><strong>${item.idPesanan}</strong></td>
                ...
                <td class="action-buttons">
                    <button class="copy-do-btn" data-do="${item.idPesanan}">Salin</button>
                    <button class="use-do-btn" data-do="${item.idPesanan}">Gunakan</button>
                </td>
            `;
...
function setupDummyTableButtons() {
    ...
    useButtons.forEach(button => {
        button.addEventListener('click', function() {
            const idPesanan = this.getAttribute('data-do');
            const doInput = document.getElementById('doNumber');
            
            if (doInput) {
                doInput.value = idPesanan;
                doInput.focus();
